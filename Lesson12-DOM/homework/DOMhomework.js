...
createdBy.appendChild(document.createTextNode(" Jay"));
/* 
...
function buildToDo(ToDo, index) {
  ...
  if(ToDo.complete === true){
    toDoText.class.add('completeText');
...
function buildToDos(toDos) {
  // code here
  let outputarr = toDos.map(buildToDo);
  return outputarr;
}
...
function addToDo() {
  ...
  let descrip = newToDo.value;
  let todo1 = new ToDo(descrip);
